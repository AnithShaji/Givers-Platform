...
namespace e_Welfare.BLL.BusinessObjects
{
    /// <summary> 
    ...
    public class ManageUserLogin : IManageUserLogin
    {
        ...
        {
            var user = this.unitOfWork.UserRepository.GetQuery(u => u.Username == userName && u.Password == password && u.UserStatus.UserStatusCode == "ACTV").FirstOrDefault();
        ...
        public Users AuthenticateTemp(string userName, string password)
        {
            ...
            var user = this.unitOfWork.UserRepository.GetQuery(u => u.Username == userName && u.Password == password);
